...
- **The desktop 188px gutter now applies from `lg` (1024px) instead of `md` (768px)** — below `lg`, `content` falls back to the mobile gutter (15px). The fixed 188px gutter left very little room (768−2×188=392px) for tablet-width (768–1023px) grids like Pickups' multi-column rows; pushing the switch to `lg` gives that range the full-width mobile-gutter room while keeping the 188px gutter for genuinely wide desktop layouts.
